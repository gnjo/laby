...
１．階層化できる。始点と終点を指定できる為。
２．全ての道と部屋が繋がる。
３．接続点は必ず移動できる事が保証されている。生成ロジックが実際に歩いている為。
４．再現性がある。乱数生成器も含め、シード値を固定すれば、計算機や言語に依存せず、必ず同一。
５．十分な複雑さがある。部屋をランダムに設置し、そこから道に繋げる為。
６．ドアがドア条件を満たす。

＊ドア条件：020並びが縦か横に必ず現れる。窪みにあるドアの形。
```
